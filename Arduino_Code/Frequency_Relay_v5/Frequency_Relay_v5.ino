...
const int relay = 3;
const int waitPeriod = 500; // The time to wait (in milliseconds)
...
//int pulseCount = 0; // This holds the pulses from the sensor
...
{
  //detachInterrupt(0);
  counter = waitPeriod; // Reset the counter
...
void setup() {
  ...
  // Add the interrupt
  // This never needs to be detached?
  attachInterrupt(0, pinInterrupt, FALLING);
}
...
void loop() {
  ...
  digitalWrite(LED_BUILTIN, HIGH);  // Set the LED/Relay high
  digitalWrite(relay, HIGH);  // Set the LED/Relay high
  ...
  digitalWrite(LED_BUILTIN, LOW);  // Set the LED/Relay low
  digitalWrite(relay, LOW);  // Set the LED/Relay high
